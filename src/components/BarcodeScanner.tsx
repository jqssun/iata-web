...
export default function BarcodeScanner({ onScan, isLoading }: BarcodeScannerProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    setInput(sanitizeBarcodeInput(e.target.value));
    _autoResize(e.target);
  };

  const _autoResize = (el: HTMLTextAreaElement) => {
  ...
  }, [isPaused, isScanning, onScan, scanBarcodeFromImage, stopScanning]);
  ...
  return (
    <div className="govuk-!-margin-top-6" style={{ maxWidth: '100%' }}>
          ...
          <div style={{ position: 'relative', backgroundColor: '#000000', borderRadius: '8px', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', top: '15px', right: '15px', display: 'flex', gap: '8px' }}>
              <button
                ...
                title={isPaused ? "Resume scanning" : "Pause and capture"}
              >
      ...
      <form onSubmit={handleSubmit} className="govuk-form-group">
        <label htmlFor="barcode-input" className="govuk-label">
          Boarding pass barcode text
        </label>
        <div className="govuk-hint">
          It typically begins with an "M" followed by a number: for example, "M1..."
        </div>
        <textarea
          ref={textareaRef}
          id="barcode-input"
